...
for(j in 1:length(unique(ds1.in$fcode))){
  ...
  for(i in 1:length(dates.test)){
    ...
    ds1.test_quant <- ds1.in.dist.init[[j]] %>%  #note uses whole data, not just most recent 5
      filter(date < X & fcode==dist.select & monthN == month.test ) %>% #& epidemic_flag <=0 ) %>% #select the same month for all years, excluding past epidemics
      summarize(n_times=n(),
    ...
    # FIX: use any() instead of sum() for clarity; guard empty most.recent
    if(nrow(most.recent)==0 || !any(most.recent$obs_dengue_cases>0)){
      ...
      # FIX: rpois and rnorm must use the same N; was rpois(10000,...) + rnorm(1000,...) — mismatch
      pois.ucl= quantile(rpois(10000, lambda=exp(mod1.coef['Estimate'] +rnorm(10000,mean=0, sd=mod1.coef['Std. Error']))), probs=0.975)
    }
    
    # FIX: guard against empty most.recent (mean of zero rows = NaN -> NA flag downstream)
    if(nrow(most.recent)==0){
      pois.ucl2 <- 5
    }else{
    ...
    # FIX: corrected prob argument; original n/(n+1) ignores case magnitude entirely.
    # Correct formula: alpha/(alpha + mu) where alpha = size, mu = sample mean.
    # Also guard empty most.recent (original returns Inf when length=0).
    if(nrow(most.recent)==0){
      nb.ucl <- 5
    }else{
      nb.alpha <- sum(most.recent$obs_dengue_cases)+0.5
      nb.mu    <- mean(most.recent$obs_dengue_cases)
      nb.ucl <- qnbinom(0.95, size=nb.alpha, prob=nb.alpha/(nb.alpha + nb.mu))
    }
...
# FIX: y=obs_dengue_cases on main aes but y=obs_inc in geom_point — mixed scales on same axis.
# Changed main aes to obs_inc to be consistent.
ggplot(ds2_pois10, aes(x=date, y=obs_inc)) +
  geom_line(col='black') +
  theme_classic() +
...
# FIX: na.rm=TRUE added to all; without it a single NA flag returns NA silently
mean(ds2_pois$epidemic_flag_poisson,  na.rm=TRUE)
mean(ds2_pois$epidemic_flag_poisson2, na.rm=TRUE)
mean(ds2_pois$epidemic_flag_nb,       na.rm=TRUE)
mean(ds2_pois$epidemic_flag_quant,    na.rm=TRUE)
mean(ds2_pois$epidemic_flag,          na.rm=TRUE)
mean(ds2_pois$epidemic_flag_fixed,    na.rm=TRUE)
