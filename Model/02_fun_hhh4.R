hhh4_mod <- function(vintage_date, modN,max_horizon=3){
 ...
 #sim.mat <- readRDS('./Data/tsclust_simmat.rds')
  ...
  # Print current neighbor status (before adding)
  #cat("\nBefore adding neighbors:\n")
  #cat("KIEN_HAI (", kien_hai_idx, ") neighbors:", which(neighb[kien_hai_idx, ] == 1), "\n")
  #cat("PHU_QUOC (", phu_quoc_idx, ") neighbors:", which(neighb[phu_quoc_idx, ] == 1), "\n")
  #cat("RACH_GIA (", rach_gia_idx, ") neighbors:", which(neighb[rach_gia_idx, ] == 1), "\n")
  
  # Add neighbors manually (adjacency matrix is symmetric, so we need to update both directions)
  
  # Make KIEN_HAI and RACH_GIA neighbors
  neighb[kien_hai_idx, rach_gia_idx] <- 1
  neighb[rach_gia_idx, kien_hai_idx] <- 1
  
  # Make PHU_QUOC and RACH_GIA neighbors
  neighb[phu_quoc_idx, rach_gia_idx] <- 1
  neighb[rach_gia_idx, phu_quoc_idx] <- 1
  
  # Make KIEN_HAI and PHU_QUOC neighbors to each other
  neighb[kien_hai_idx, phu_quoc_idx] <- 1
  neighb[phu_quoc_idx, kien_hai_idx] <- 1
  

  
  # Calculate distance-based neighbor order
